...
async function createRecord(req, res) {
    if (schema.validate(req.body.password)) {
            ...
            if (err)
                ...
                try {
                    ...
                    mailer.sendMail({
                        ...
                        to: data.email,
                        subject: "Account Has Been Created : Team OwnChoice",
                        text: `
        ...
        res.send({ result: "Fail", reason: "Invalide Password! Is Must Contains Atleast 1 Upper Case Alphabet, 1 lower Case Alphabet, 1 Digit, Doesn't Contains Any Space And Length Must Be 8-100" })
    ...
    try {
        ...
        if (data) {
            ...
            mailer.sendMail({
                from: process.env.MAIL_SENDER,
                to: data.email,
                subject: "OTP For Password Reset : Team OwnChoice",
                text: `

                        Hello ${data.name},
                        OTP For Password Reset Is ${otp}
                        Never Shere OTP With Anyone
                        Team : OwnChoice
                `
            }, (error) => {
                console.log(error);
            ...
            if (schema.validate(req.body.password)) {
                bcrypt.hash(req.body.password, 12, async function (err, hash) {
                    if (err)
                        ...
                        data.password = hash
                        await data.save()
                        res.send({ result: "Done", message: "Youe Password Has Been Reaset Successfully" })
                    }
                })
            }
            else
